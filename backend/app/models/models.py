...
# FIX #19: Added index=True to ALL foreign key columns.
# Previously ALL 21 FK columns had no database index,
# causing full table scans on every JOIN/filter operation.
...
class Client(Base):
    ...
    # Linked enquiry
    enquiry_id = Column(Integer, ForeignKey("enquiries.id"), unique=True, nullable=False, index=True)
...
class WorkflowProgress(Base):
    ...
    template_stage_id = Column(Integer, ForeignKey("workflow_templates.id"), nullable=False, index=True)
...
class Director(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    company_id = Column(Integer, ForeignKey("company_master.id"), nullable=False, index=True)
...
class ShareTransfer(Base):
    ...
    from_shareholder_id = Column(Integer, ForeignKey("shareholders.id"), nullable=True, index=True)
    to_shareholder_id = Column(Integer, ForeignKey("shareholders.id"), nullable=True, index=True)
