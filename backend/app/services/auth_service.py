...
# FIX #8: Removed unused BackgroundTasks re-export — it was being imported
# from here in meetings.py which is fragile. meetings.py now imports directly
# from fastapi. Kept the import removed to prevent confusion.
...
def ensure_super_admin(db: Session):
    ...
    FIX #9: Default password now read from env var SUPER_ADMIN_DEFAULT_PASSWORD
    instead of being hardcoded in source code.
    Default credentials (change immediately after first login):
      email: admin@taxly.com
      password: set via SUPER_ADMIN_DEFAULT_PASSWORD env var (fallback: Admin@1234)
    """
    import os
    ...
    default_pwd = os.getenv("SUPER_ADMIN_DEFAULT_PASSWORD", "Admin@1234")
    pwd_hash, salt = hash_password(default_pwd)
    admin = UserAccount(
        name="System Administrator",
        email=os.getenv("SUPER_ADMIN_EMAIL", "admin@taxly.com"),
    ...
    db.add(admin)
    db.commit()
    print(f"✅ Default Super Admin created: {admin.email} / [see SUPER_ADMIN_DEFAULT_PASSWORD env var]")
