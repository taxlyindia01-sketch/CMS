...
def dashboard_stats(request: Request, db: Session = Depends(get_db)):
    ...
    # FIX #14: Replaced N+1 (iterating s.clients per staff row) with a single
    # SQL subquery that counts active clients grouped by assigned_staff_id.
    client_count_subq = (
        select(
    ...
    staff_rows = (
        db.query(Staff, func.coalesce(client_count_subq.c.cnt, 0).label("client_count"))
    ...
    return {
        ...
        "staff_workload": [
            {"name": s.name, "designation": s.designation, "client_count": cnt}
            for s, cnt in staff_rows
        ],
    }
