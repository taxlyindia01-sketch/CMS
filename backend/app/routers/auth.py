...
All HTML routes serve Index.html (self-contained SPA — no Jinja2 templates needed).
"""
...
def _serve_spa() -> FileResponse:
    """Return the SPA HTML file for all page routes."""
    return FileResponse(str(_SPA_FILE), media_type="text/html")


# ── HTML Page Routes — all serve the same SPA ──────────────────────────────────

@router.get("/", response_class=HTMLResponse)
def landing_or_redirect(request: Request, db: Session = Depends(get_db)):
    """Root: redirect authenticated users to /app, else serve SPA (shows landing)."""
...
def spa_page(request: Request):
    """Main SPA entry point — serves Index.html for all app routes."""
    return _serve_spa()
